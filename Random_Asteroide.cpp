...
Random_Asteroide::Random_Asteroide(int s, int dir) : direccion(dir){}

void Random_Asteroide::inicializaRandom_Asteroide(float x, float y, int s){
    //Creamos el Random_Asteroide
    //Unicamente dibujaremos un Random_Asteroide dependiendo el tamaño que se le ingreso
	//GUARDAMOS LOS EJES. YA QUE EN TEORIA EL Random_Asteroide ESTARA EN 0,0, Y UNICAMENTE SE DESPLAZA A SU EJE AL IMRPIMIRSE
	ejeX = x;
	ejeY = y;
	size = s;
	//size = 1;
    ...
    switch(size){
        case 1: //GRANDE
			//Haremos el Random_Asteroide
			xInit = getRnd(50,110);
			yInit = getRnd(50,110);
            pts[0].x= - xInit; pts[0].y= yInit; //Haremos cordenadas Random, para tener siempre Random_Asteroides diferentes
...
void Random_Asteroide::imprimeRandom_AsteroideColor(){
	gfx_color(0,200,100);
	for (int i = 0; i < 9; ++i){
	...
	gfx_flush();
	usleep(TIEMPO);
	imprimeRandom_AsteroideNegro();
	return;
}

void Random_Asteroide::imprimeRandom_AsteroideNegro(){
	gfx_color(0,0,0);
	for (int i = 0; i < 9; ++i){
		gfx_line(pts[i].x+ejeX,pts[i].y+ejeY,pts[i+1].x+ejeX,pts[i+1].y+ejeY);
	}
...
void Random_Asteroide::mueveRandom_Asteroide(int d, int e){
    direccion = d;
    eje = e;
	imprimeRandom_AsteroideColor();
	rotarEje(1.0);
...
void Random_Asteroide::rotarEje(float angulo) {
